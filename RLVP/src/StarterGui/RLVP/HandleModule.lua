...
local cMode, mBind, setT
local binds = {}
local adornee
local specOrig
local isPrev

local incrT = {move = 'lat', resize = 'lat', rotate = 'rot'}
...
function drag(which, m)
	...
	local min, max = setT.min, setT.max
	if min and m < min then m = min end
	if max and m > max then m = max end

	if magn == m then return end
	gui.action('value', 'display', m)
	tool.drag(which, m, magn)
	magn = m
end
...
function setPart(p)
	...
	elseif currH then
		specOrig = ogMod.process(currP, mBind.origin)
...
return {
	...
	getCFrame = function() return adornee.CFrame end,
	setPart = setPart
}
...
function set(modeT, ...)
	...
	tagT[o] = tagT[o] or {origin = o, tagged = false}
	...
	local ret = {}
	if bind.limit then ret.min, ret.max = bind.limit(o, tagT, ...) end
	return ret
end
...
function unset()
	...
	for p, t in next, tagT do
		local nP, par = p, p.Parent
		if bind.output and t.tagged then nP = bind.output(true, p, t) end
		tagT[p] = nil
...
function exec(...)
	if bind and tagT then for p, t in next, tagT do bind.drag(true, p, t, ...) end end
end
	...
	if not tagT then
		...
		if bind.input then
			...
			for i, p in next, t do
				...
				-- Sets the object up.
				local nP, tags = bind.input(false, p, ...)
				...
				-- Executes the object.
				bind.drag(false, nP, tags, ...)

				-- Unsets the object.
				nP = bind.output(false, p, tags)

				if p ~= nP then repT[p] = nP end
			end

			if not originDone then
				bind.drag(o, {isOrigin = true, inSelection = true, tagged = false}, ...)
			end
			...
			for i, p in next, t do
				bind.drag(p, {isOrigin = p == o, inSelection = true, tagged = false}, ...)
			end
		end
	else
		exec(...)

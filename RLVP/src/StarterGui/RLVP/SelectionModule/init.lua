...
function callGroupFunc(partI, name, ...)
	...
	local g = selGroups[partI]
	local f = g.algor[name]
...
function replaceSelectionGlobal(t)
	...
	for m, g in next, selGroups do
		...
		if ds and ds ~= pt then for i, gp in next, ds do ds[i] = t[gp] or ds[i] end end
	...
	for i, p in next, pluginSels do pluginSels[i] = t[p] or pluginSels[i] end

	-- Redirects both sleection tables.
	for i = #studioSels, 1, -1 do
		pluginSels[i] = t[pluginSels[i]] or pluginSels[i]
		studioSels[i] = t[studioSels[i]] or studioSels[i]
	end
	...
	for p1, p2 in next, t do
		if not grRedirs[p1] and not inGrT[p1] then
			createGroup(p2)
			grRemove[p2] = true
		end
	end
...
function shift(p)
	...
	for i = 2, #pluginSels do
		if pluginSels[i] == p then
			...
			pluginSels[i] = pluginSels[1]
			pluginSels[1] = p

			-- Shifts the corresponding studio selections.
			studioSels[i] = studioSels[1]
			studioSels[1] = p
...
-- Self-explanitory.
function getStudioSels() return game.Selection:Get() end
...
function changeSel(changes)
	...
	for m, g in next, selGroups do
		...
		elseif changes[m] == 'add' then
			changes[m] = nil
			for i, gp in next, g.defSel do changes[gp] = 'add' end
		end
	end
	...
	for m, g in next, selGroups do
		...
		if u then
			...
			local all = true
			for i, gp in next, g.parts do
				if changes[gp] and 'remove' ~= changes[gp] then
					all = false
					break
				end
			end
			...
			if all then
				for i, gp in next, g.parts do
					if 'remove' == changes[gp] then changes[gp] = nil end
				end
				grRemvd = true
				removeGroup(m)
...
evtMod.bind('negateSel', function(addT, remT)
	...
	for m, g in next, selGroups do
		if not callGroupFunc(m, 'modify', 'negateSel', addT, remT) then
			remGrT[m] = true
		end
	end
...
evtMod.bind('separSel', function(addT, remT)
	...
	-- Attempts to send event data to each group; will modify addT and remT.
	for m, g in next, selGroups do
		if not callGroupFunc(m, 'modify', 'separSel', addT, remT) then
			remGrT[m] = true
		end
	end
...
evtMod.bind('restore', function(wp)
	...
	for i, p in next, tempSS do if algorSel(p) then algorIndexT[p] = true end end
	...
	-- Create groupss
	for p in next, algorIndexT do createGroup(p) end
	send()
end)
